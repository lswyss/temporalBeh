...
def data_of_interest(names, interest=[], exclude=[]):
    # Converts to_plot and exclude to sets for faster lookup
    to_plot = set()
    exclude = set(exclude)
    ...
    for dat in names:
        if any(i in dat for i in interest):
            # Excludes any data that matches the exclude criteria or if double/triple knockdown
            if any(ex in dat for ex in exclude) or any(dat.count('+') > i.count('+') for i in interest):
                continue
            to_plot.add(dat)

    return list(to_plot)  # Convert back to list if necessary


def calc_delta(x, w=2):
    # Adjust the window to be odd
    if w % 2 == 0:
        w += 1

    # Create filter array for convolution
    filt = np.ones(w)
    filt[:w // 2] = -1
    filt[w // 2] = 0

    # Apply convolution and return the result
...
def response_sin(interest_list,exclude,periods =[.5,1,2,3,4,5,10],duration=30,
    ...
    for ii,interest in enumerate(interest_list):
        ...
        name = 'data/LDS_response_sinFunc_v3.pickle'
        #name = 'data/LDS_response_sinFunc_v2.pickle'
        if plot_indiv_trial:
            name = 'data/LDS_response_sinFunc_individual_v2.pickle'
        if visible:
        ...
        for i,p in enumerate(periods):
         ...
         # Start with minutes
            ...
            if plot_indiv_trial:
                ...
                if derivative:
                    # Placeholder for derivative related code, if any.
                    # The 'True' statement is redundant and can be removed.
                    c = plt.cm.Set1(ii / 9)  # Color assignment based on index
                else:
                    # Default color if derivative is not required
                    c = 'grey'
                ...
                # Plot additional stimulus data
                ax[i, 0].plot(xp, result[to_plot[0]]['stim'], c='thistle', zorder=-10)

                # Time-dependent significance calculation
                time_sig = timeDependentDifference(yp[:, ind_t], yp_ref[:, ind_t_ref], n_boot=n_boot, conf_interval=conf_interval)
                x_sig = xp[ind_t]
                y_sig = .1
                bott = np.zeros_like(x_sig) + 1.5

                # Fill between for significance
                ax[i, 0].fill_between(x_sig, bott, bott - y_sig * time_sig, facecolor=c, alpha=.4)

                # Drawing significance boxes
                box_keys = {'lw': 1, 'c': 'k'}
                ax[i, 0].plot(x_sig, bott, **box_keys)
                ax[i, 0].plot(x_sig, bott - y_sig, **box_keys)

            # Add legend to the current axis
            ax[i, 0].legend()
    ...
    # Set label for the last axis in the plot
...
def response_sin_powerspectrum(interest,exclude,periods =[.5,1,2,3,4,5,10],duration=30,
    ...
    for i,p in enumerate(periods):
        ...
        print(to_plot)
        xp = result['tau']
        yp = [result[dat]['data'] for dat in to_plot]
        if len(yp)==0:
            continue
        yp=np.concatenate(yp)
        # tau = np.arange(1,int(p*120))
        # print(yp.shape,result[to_plot[0]]['stim'][ind_t].shape)
        from tools.measurements import power_spectrum
        results = [power_spectrum(yy, sampling_rate=120) for yy in yp[:,ind_t]]
        C = np.array([dat[1] for dat in results])
        ...
        ax.fill_between(1/xx,*rng,facecolor=c,alpha=.2)
    ax.legend()
    ax.set_xlim(0,20)
...
#Oscillations Not working yet: LW 01.31.24
...
# def response_sin_regen(interest_list, exclude, periods=[.5,1,2,3,4,5,10], duration=30,
#                        n_boot=1e3, statistic=np.median, conf_interval=95, 
#                        stat_testing=True, visible=False, t_samp=(-3,40),
#                        plot_indiv_trial=False):

#     # Load the regenerating sine wave data
#     name = 'data/LDS_response_sinFunc_regen.pickle'
#     with open(name, 'rb') as f:
#         result = pickle.load(f)

#     # Assume 'days' key contains the list of days in the dataset
#     days = result['days']  # This needs to be adjusted based on your data structure

#     # Initialize the figure with subplots for each day
#     fig, ax_all = plt.subplots(nrows=len(days), ncols=len(periods), figsize=(10, 2 * len(days) * len(periods)))
#     if len(days) == 1:
#         ax_all = np.array([ax_all]).T  # Ensure ax_all is a 2D array

#     # Loop over days and periods
#     for day_idx, day in enumerate(days):
#         for period_idx, p in enumerate(periods):
#             ax = ax_all[day_idx, period_idx]

#             # Process and plot the data for each day and period
#             # [Include your data processing and plotting logic here]
#             # For example:
#             # xp, yp, stimulus = process_day_period_data(day, p, result, interest_list, exclude, ...)
#             # ax.plot(xp, yp, ...)
#             # ax.plot(xp, stimulus, ...)

#             # Set titles, labels, etc.
#             ax.set_title(f'Day {day}, Period {p}m')
