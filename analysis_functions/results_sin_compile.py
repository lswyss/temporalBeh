...
def response_sin(interest_list, exclude, periods, duration, n_boot, statistic, conf_interval, t_samp, amplitudes=None, return_data=False):
    ...
    for i, p in enumerate(periods):
        ...
        for interest in interest_list:
            for amplitude in amplitudes:
                ...
                # Store data internally if requested
                if return_data:
                    data_storage[exp] = {
                        'stim': stim_data,
                        'activity': y,
...
#         xp = result['tau']
#         ind_t = np.where((xp >= t_samp[0] - 1e-5) & (xp <= t_samp[1]))[0]
#         p_name = f'{p}mPeriod'  # Periodicity name to match in experiment
...
#                 # Concatenate data for all matching experiments with the same periodicity
#                 yp = np.concatenate([result[dat]['data'] for dat in to_plot])
#                 y, rng = bootstrap_traces_sam(yp[:, ind_t], n_boot=n_boot, statistic=statistic, conf_interval=conf_interval)
#                 stim_data = result[to_plot[0]]['stim'][ind_t]

#                 # Plot the aggregated data for this periodicity
#                 plot_sin(ax_all[i, 0], xp[ind_t], y, rng, f'{interest} {p}m, {amplitude}, ({yp.shape[0]})', color='cornflowerblue', plot_stim=True, stim_data=stim_data)

#                 # Store aggregated data by periodicity (e.g., "3m", "5m") instead of individual experiment names
#                 period_key = f"{p}m"
#                 if period_key not in data_storage:
#                     data_storage[period_key] = {
#                         'stim': stim_data,
#                         'activity': y,
#                         'rng': rng,
#                         'xp': xp[ind_t]
#                     }
#                 else:
#                     # Optionally, you could handle multiple entries per period differently if needed
#                     print(f"Warning: Data for {period_key} already exists, check for duplicates.")
